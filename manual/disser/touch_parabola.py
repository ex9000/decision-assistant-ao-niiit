...
sol = solve_frontier(sysmatrix, {2: 0})
(left, right), *_ = intersect_segments(
    apply_gt_constraints(sol, np.zeros(size)),
    apply_lt_constraints(sol, np.ones(size)),
)
d = right - left
xs = np.linspace(left - 0.1 * d, right + 0.1 * d, 1000)
rest = xs  # [(xs > left) & (xs < right)]
weights = polyval(rest, sol)
ax.plot(rest, quadratic_form(covariance, weights), c="k", linewidth=5, zorder=-2)

rest = xs[(xs > left) & (xs < right)]
weights = polyval(rest, sol)
ax.plot(rest, quadratic_form(covariance, weights), c="lime", linewidth=3, zorder=-1)
